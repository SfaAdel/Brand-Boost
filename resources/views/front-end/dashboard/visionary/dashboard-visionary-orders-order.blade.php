...
<div class="bg-pr text-white border rounded-lg border-gray-200 h-full">
    <div class="p-5 flex flex-col gap-5">
        ...
        <div class="grid grid-cols-2 gap-5">
            ...
            <!-- Remaining Time -->
            <div>
                <h2 class="text-lg">{{__('website.remaining_time')}}</h2>

                @if ($order->status == 'complete')
                    <p class="text-sm "> {{__('website.delivered_successfully')}}</p>
                @elseif ($remainingTime->invert)
                    <p class="text-sm text-red-500">{{__('website.overdue')}} {{ $remainingTime->days }}
                        {{__('website.days')}}
                    </p>
                @else
                    <p class="text-sm">
            ...
            <!-- Deadline -->
            <div>
                <h2 class="text-lg">{{__('website.deadline')}}</h2>
            ...
            <!-- Payment Status -->
            <div>
                <h2 class="text-lg">{{ __('Payment Status') }}</h2>
        ...
        text-white py-0.5 px-2 text-[11px] font-medium w-fit">
            ...
            <!-- Order Status -->
            <div>
                <h2 class="text-lg">{{ __('Order Status') }}</h2>
                <div class="relative grid items-center uppercase whitespace-nowrap select-none 
        ...
        text-white py-0.5 px-2 text-[11px] font-medium w-fit">
            ...
            <div>
                <h2 class="text-lg">{{__('website.amount')}}</h2>
                <p class="text-sm">{{ $order->amount }}</p>
            </div>
            <!-- Status -->
            <div>
                <h2 class="text-lg">{{__('website.total_price')}}</h2>
                <p class="text-sm">{{ $order->total_price }}</p>

            </div>

            <div>
                <h2 class="text-lg">{{__('website.freelancer')}}</h2>
            ...
            <!-- Description -->
            <div>
                <h2 class="text-lg">{{__('website.description')}}</h2>
